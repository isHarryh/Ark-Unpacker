...
WebUI 可以浏览已解包的图片、音频、视频、文本和模型，也可以直接打开 `.ab` / `.bin` 文件查看其中的 Unity 对象并按需预览或下载，无需先完整解包。

- 点击资源列表的“名称 / 类型 / 大小 / 修改时间（或 PathID）”表头可以切换升序和降序；
- 拖动预览栏左侧边缘可以调整宽度，双击边缘可恢复默认宽度；
- 点击顶栏“设置”可以在 85%–150% 范围内调整界面字号。字号和预览栏宽度会保存在当前浏览器中。
- `.usm` 视频会在首次预览时临时转换为浏览器兼容的 MP4，后续播放复用缓存；此功能需要系统中可以调用 FFmpeg。
- 多层 Bundle 中的对象使用“内层来源文件 + 对象”复合定位，避免不同层级的重复 PathID 冲突；悬浮信息中可以查看并搜索内层来源。
- 下载 `.ab` / `.bin` 时会先自动导出支持的资源，按内层来源整理后生成 ZIP；ZIP 中包含 `_arkunpacker_manifest.json` 导出清单。

```shell
# 从当前目录启动，自动选择空闲端口并打开浏览器
python Main.py --webui

# 指定初始资源目录和端口
python Main.py --webui -i "D:\\Arknights\\Bundles" --port 8765

# 也可以作为 Python 模块单独启动
python -m src.webui "D:\\Arknights\\Bundles"
```

服务默认仅监听 `127.0.0.1`，在终端按 `Ctrl+C` 即可停止。界面顶部可以随时切换资源根目录；后续所有文件访问都被限制在该根目录内。
