...
3. **时效性警告** - 以下内容基于 **Android`v1.9.01`** 的明日方舟，不同版本的资源具体位置可能存在差异。
...
### Activity
- `activity\commonassets.ab` 活动物资相关图标
...
- `audio\sound_beta_2\voice_xx\` 语音包(其他语言)
...
### Hotupdate
- `hotupdate\[uc]worldtips.ab` 热更新时背景图
...
- `spritepack\ui_deep_sea.ab` 活动:愚人号相关
...
保全派驻
- `spritepack\ui_climb_tower_bkg.ab` 保全派驻:背景图
- `spritepack\ui_climb_tower_stage_icon.ab` 保全派驻:关卡封面
- `spritepack\ui_climb_tower_zone_icon.ab` 保全派驻:地区图标
...
- `spritepack\ui_zone_home_theme.ab` 终端封面
...
- `ui\zonemap_x.ab` 主线关卡页背景
- `ui\zonemap_campx.ab` 剿灭作战关卡页背景
